...
def load_Color(path):
    """Dataset Color: MPEG-7 features numéricas CON header (dim n p)."""
    with open(path, "r") as f:
        header = f.readline().strip().split()
        dim = int(header[0])   # 282 en tu Color_2k

        rows = []
        for line in f:
            parts = line.strip().split()
            # nos aseguramos de tomar sólo filas con la dimensión correcta
            if len(parts) == dim:
                try:
...
def process_dataset(name, path, outdir="prepared_experiment"):
    ...
    os.makedirs(f"{outdir}/pivots2k", exist_ok=True)
    os.makedirs(f"{outdir}/queries2k", exist_ok=True)
    os.makedirs(f"{outdir}/radii2k", exist_ok=True)

    # QUERIES
    queries = select_queries(N, 100)
    with open(f"{outdir}/queries2k/{name}_queries.json", "w") as f:
        json.dump(queries, f, indent=2)

    # RADIOS
    selectivities = [0.02, 0.04, 0.08, 0.16, 0.32]
    radii = compute_radii(data, queries, metric, selectivities)
    with open(f"{outdir}/radii2k/{name}_radii.json", "w") as f:
        json.dump(radii, f, indent=2)

    # PIVOTES
    for p in [3, 5, 10, 15, 20]:
        pivots = select_pivots_HFI(data, p, metric)
        with open(f"{outdir}/pivots2k/{name}_pivots_{p}.json", "w") as f:
