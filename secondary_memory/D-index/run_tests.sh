#!/bin/bash
# run_tests.sh
# Script para ejecutar tests de D-index en Linux/WSL
# Uso: ./run_tests.sh

echo ""
echo "==== D-Index Test Runner ===="
echo "Configuración: 4 niveles, rho=5.0"
...
if [ ! -f "test.cpp" ]; then
    ...
    echo "Asegúrate de estar en el directorio secondary_memory/D-index"
    exit 1
fi

# Compilar si es necesario
if [ ! -f "DIndex_test" ]; then
    ...
    echo "[OK] Ejecutable encontrado: DIndex_test"
fi

# Iniciar ejecución en background
echo ""
echo "[START] Iniciando ejecución en background..."
LOGFILE="dindex_run_$(date +%Y%m%d_%H%M%S).log"
nohup ./DIndex_test > "$LOGFILE" 2>&1 &
PID=$!
echo $PID > dindex.pid
...
echo "  LA:        ~45-60 minutos"
echo "  Words:     ~25-35 minutos"
echo "  Synthetic: ~3-5 minutos"
echo "  TOTAL:     ~75-100 minutos"
