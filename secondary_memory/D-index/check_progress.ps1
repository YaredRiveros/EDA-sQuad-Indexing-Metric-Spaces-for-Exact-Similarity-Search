# check_progress.ps1
# Script para monitorear el progreso de D-index con auto-refresh
# Uso: .\check_progress.ps1

Write-Host "`n==== D-Index Progress Monitor ====" -ForegroundColor Cyan
...
try {
    while ($true) {
        ...
        if (!$job) {
            Write-Host "`n[WARN] No se encontró job 'DIndex'" -ForegroundColor Yellow
            ...
            if (Test-Path "dindex_run.log") {
                ...
                Get-Content "dindex_run.log" -Tail 30 | ForEach-Object {
        ...
        if ($output) {
            ...
            foreach ($line in $output) {
                ...
                if ($line -match "Cargados|Indexados|BUILD") {
                ...
                elseif ($line -match "stats|buckets|Total") {
