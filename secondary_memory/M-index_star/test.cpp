...
void test_dataset(const string& dataset) {
    ...
    if ((int)pivots.size() < NUM_PIVOTS_DISK) {
        cout << "[ERROR] Pivotes HFI para P=5 no disponibles o incompletos. Abort.\n";
        J << "\n]\n";
        J.close();
        return;
    }
    pivots.resize(NUM_PIVOTS_DISK);

    MIndex_Improved midx(db.get(), NUM_PIVOTS_DISK);
    midx.overridePivots(pivots);
    string base = "midx_indexes/" + dataset + "_p5";
