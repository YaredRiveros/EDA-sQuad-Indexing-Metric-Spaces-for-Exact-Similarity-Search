...
export default function MRQChart({ data, memory = "main", query = "MRQ" }) {
  ...
  // decidir si y va en log o lineal (igual que en main)
  let yType = "linear";
  if (allY.length > 1) {
  ...
  const xScale = buildXScale({
    memory,
    query,
    label: memory === "main" ? "L" : "X",
  });

  return (
    <ChartContainer title="Running Time (ms)">
      <Line
        ...
        options={{
          ...
          scales: {
            ...
            y: {
              type: yType,
              title: { display: true, text: "ms", color: "#ccc" },
              ticks: { color: "#ccc" },
              grid: { color: "#ffffff10" },
            },
          },
