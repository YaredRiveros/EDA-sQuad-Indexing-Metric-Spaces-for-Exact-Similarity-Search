...
export default function MkNNChart({ data, memory = "main", query = "MRQ" }) {
  const indices = Object.keys(data || {});
  const allY = [];

  const series = indices.map((idx) => {
    const pts = data[idx] || [];
    pts.forEach((p) => allY.push(p.comp));

    return {
      label: idx,
      data: pts.map((p) => ({ x: p.x, y: p.comp })),
  ...
  return (
    <ChartContainer title="compdists">
      <Line
        ...
        options={{
          ...
          scales: {
            ...
            y: {
              type: yType,
              title: { display: true, text: "comparaciones", color: "#ccc" },
              ticks: { color: "#ccc" },
              grid: { color: "#ffffff10" },
            },
          },
