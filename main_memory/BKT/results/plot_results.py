...
for dataset in DATASETS:
    filename = f"results_BKT_{dataset}.json"
    ...
    plt.title(f"{dataset} — BKT MRQ Running Time")
    plt.grid(True)

    plt.savefig(f"{PLOT_DIR}/BKT_{dataset}_MRQ_time.png", dpi=200)
    plt.close()
    ...
    plt.title(f"{dataset} — BKT MRQ compdists")
    plt.grid(True)

    plt.savefig(f"{PLOT_DIR}/BKT_{dataset}_MRQ_compdists.png", dpi=200)
    plt.close()
